...
const SucessModal = ({onResultChange ,message, title}:modalProps) => {
      ...
      <div className={`${visible ? 'opacity-100 transition-all ease-linear duration-300' : 'opacity-0 transition-all ease-linear duration-150'}`}>
            <div
                ...
                <div className="my-8 text-center">
                    <svg  xmlns="http://www.w3.org/2000/svg" className="animate-[bounce_2s_ease-in-out_infinite] w-16 shrink-0 fill-green-500 inline" viewBox="0 0 512 512">
                    <path
                ...
                <button type="button"
                    className="px-6 py-2.5 min-w-[150px] w-full rounded text-white text-sm font-semibold border-none outline-none bg-blue-600 hover:bg-blue-800" onClick={handleResultChange}>Ok</button>
                </div>
            </div>
        </div>
    );
